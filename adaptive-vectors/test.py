...
def evaluation(model_args, data_args, training_args):
    ...
    ans_pred_list = []
    # fixed_k_eval: predictions decoded at each forced prefix length k (1..k_max).
    # fixedk_preds[k] accumulates one prediction per example, in dataset order.
    fixedk_preds = None
    ...
    for step, batch in enumerate(question_data):
        ...
        with torch.no_grad():
            ...
            if training_args.fixed_k_eval:
                # --- Diagnostic: force-decode the answer at every prefix k=1..k_max ---
                # Ignores the halting head entirely. The latent loop advances the shared
                # cache normally; at each step we decode an answer from an *independent
                # copy* of the cache (generate_answers mutates DynamicCache in place), so
                # the loop continuation is uncorrupted. Yields accuracy@k after the loop.
                k_max = model.max_latent_steps
                if fixedk_preds is None:
                    fixedk_preds = [[] for _ in range(k_max)]
                ...
                for i in range(k_max):
                    ...
                    if training_args.use_prj:
                        latent_embd = model.prj(latent_embd)
                    # decode answer at this k from a throwaway copy of the prefix
                    sub_pkv = _slice_past_key_values(past_key_values, idx_all)
                    preds_k = generate_answers(sub_pkv, attn.clone())
                    for b in range(batch_size):
                        decoded = tokenizer.decode(preds_k[b], skip_special_tokens=True)
    ...
    if training_args.fixed_k_eval:
        # Report accuracy@k for each forced prefix length and save the curve.
        os.makedirs(data_args.results_dir, exist_ok=True)
        print("\n[fixed-k] Accuracy at each forced prefix length k:")
        print(f"{'k':>3} | {'N':>6} | {'Acc (%)':>8}")
        print("-" * 24)
        curve = []
        for k in range(len(fixedk_preds)):
            preds_k = fixedk_preds[k]
            acc_k = 100.0 * compute_accuracy(answer, preds_k)
            curve.append({"k": k + 1, "n": len(preds_k), "accuracy_pct": round(acc_k, 2)})
            print(f"{k+1:>3} | {len(preds_k):>6} | {acc_k:>7.2f}%")
        curve_path = os.path.join(data_args.results_dir, f"{data_args.data_name}_fixedk_curve.json")
        write_json({"curve": curve, "ckpt": model_args.ckpt_dir}, curve_path)
        print(f"[fixed-k] curve saved to {curve_path}")
        return curve[-1]["accuracy_pct"]
