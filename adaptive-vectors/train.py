...
def train():
    ...
    if training_args.pondernet:
        # Scope-aware unfreezing (see --pondernet_train_scope):
        #   lora     : lora_* + halt_head            (warm-started backbone, the default)
        #   lora_prj : lora_* + prj.* + halt_head    (cold backbone: prj is random, must train)
        #   full     : whole codi backbone + prj + halt_head (overwrite the K-fixed prior)
