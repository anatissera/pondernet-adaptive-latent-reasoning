...
def plot_steps_by_difficulty(adapt, base, out_path, FS, k_fixed=6):
    ...
    ax.set_xticklabels(labels)
    ax.set_xlabel(r"Pasos de razonamiento del dataset (dificultad)", fontsize=FS["label"], color=TEXT)
    ax.set_ylabel("Tokens latentes usados (prom.)", fontsize=FS["label"], color=TEXT)
...
def plot_steps_and_accuracy_by_difficulty(adapt, out_path, FS):
    ...
    ax1.set_xlabel(r"Pasos de razonamiento del dataset (dificultad)", fontsize=FS["label"], color=TEXT)
    ax1.set_ylabel("Accuracy (%)", fontsize=FS["label"], color=BLUE)
    ax2.set_ylabel("Tokens latentes usados (prom.)", fontsize=FS["label"], color=ORANGE)
...
def plot_joint_matrix(adapt, out_path, FS, max_cot=7, max_steps=8):
    ...
    ax.set_xlabel("Tokens latentes usados por el modelo", fontsize=FS["label"], color=TEXT)
    ax.set_ylabel(r"Pasos del dataset (dificultad)", fontsize=FS["label"], color=TEXT)
...
def plot_steps_used_distribution(adapt, out_path, FS, k_fixed=6):
    ...
    ax.set_xlabel("Tokens latentes usados", fontsize=FS["label"], color=TEXT)
    ax.set_ylabel("Fracción del test", fontsize=FS["label"], color=TEXT)
    ax.set_xticks(xs)
    style_axes(ax, FS["tick"])
    fig.tight_layout()
    fig.savefig(out_path, dpi=200)
