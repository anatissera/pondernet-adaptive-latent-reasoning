...
def plot(counts: list[int], out_path: Path, K_max: int = 12, profile: str = "paper") -> None:
    ...
    ax.set_xlabel(r"$n_i$  (reasoning steps of the instance)", fontsize=FS["label"], color=TEXT)
    ax.set_ylabel("Fraction of the dataset", fontsize=FS["label"], color=TEXT)
    ax.set_xticks(xs)
    ...
    if over_kmax:
        ax.text(0.99, 0.95, f"{over_kmax} examples with $n_i > {K_max}$ (out of range, not shown)",
