...
def plot_accuracy(out_path: Path) -> None:
    ...
    ax.plot(KS, [a * 100 for a in ACC_FULL], color=PURPLE, marker="D",
            ...
            label=r"Full train ($n{=}7473$)")
    ax.plot(KS, [a * 100 for a in ACC_200], color=BLUE, marker="D",
            markersize=4, linewidth=1.6, linestyle="--", zorder=2,
            label=r"Pilot subsample ($n{=}200$)")

    ax.set_xlabel(r"$k$  (forced latent steps)", fontsize=10, color=TEXT)
...
def plot_kstar(out_path: Path) -> None:
    ...
    ax.set_xlabel(r"$k^*$  (instance's minimal optimal budget)",
                  fontsize=10, color=TEXT)
    ax.set_ylabel("Examples (%)", fontsize=10, color=TEXT)
    ax.set_xticks(KS)
    ax.set_ylim(0, 74)
