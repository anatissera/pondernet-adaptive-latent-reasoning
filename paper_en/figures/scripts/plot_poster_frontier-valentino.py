...
def plot(d, out_path):
    ...
    # titular
    ax.text(4.3, 40.95, "Half the compute, same accuracy",
    ...
    ax.annotate("Adaptive model", xy=(knee_s, knee_a),
    ...
    ax.text((knee_s + base_s) / 2, base_a + 0.1, "$-$50% compute",
            ha="center", va="bottom", fontsize=14, color=TEXT, fontweight="bold")

    ax.set_xlabel("Latent tokens per problem (compute)", fontsize=17, color=TEXT)
