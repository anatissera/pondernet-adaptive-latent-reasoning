...
import figstyle
# --- paleta de la casa (definida en figstyle.py) ---
BLUE = figstyle.BLUE      # adaptativo (nuestro metodo, protagonista) / eje accuracy
RED = figstyle.ACCENT     # linea de referencia (baseline constante / medias)
GRAY = figstyle.TINT      # baseline como serie real (barras): indigo palido, monocromo
ORANGE = figstyle.ACCENT  # segunda serie (eje de tokens): frambuesa, no naranja
GRID = figstyle.GRID
TEXT = figstyle.TEXT

ADAPT_LABEL = "Adaptive model"
BASE_LABEL = "Fixed-$K$ baseline"
...
def style_axes(ax, tick_size):
    ...
    ax.tick_params(colors=TEXT, labelsize=tick_size)
...
# ---------------------------------------------------------------- Figura A
def plot_accuracy_by_difficulty(adapt, base, out_path, FS):
    ...
    ax.set_xlabel(r"Dataset reasoning steps (difficulty)", fontsize=FS["label"], color=TEXT)
...
# ---------------------------------------------------------------- Figura B
def plot_steps_by_difficulty(adapt, base, out_path, FS, k_fixed=6):
    xs, labels = bin_labels()
    ...
    fig, ax = plt.subplots(figsize=(6.2, 3.6))
    idx = np.arange(len(xs))
    ax.plot(idx, ya, color=BLUE, marker="o", markersize=7.5, linewidth=2.2,
            markeredgecolor="white", markeredgewidth=1.1,
            zorder=3, label=ADAPT_LABEL)
    ax.axhline(k_fixed, color=RED, linestyle="--", linewidth=1.6, zorder=2,
               label=f"{BASE_LABEL} ($K{{=}}{k_fixed}$)")

    for i, y in enumerate(ya):
        ax.text(i, y + 0.18, f"{y:.1f}", ha="center", va="bottom",
                fontsize=FS["tick"] * 0.78, color=BLUE)

    ax.set_xticks(idx)
    ax.set_xticklabels(labels)
    ax.set_xlabel(r"Dataset reasoning steps (difficulty)", fontsize=FS["label"], color=TEXT)
    ax.set_ylabel("Latent tokens used (avg.)", fontsize=FS["label"], color=TEXT)
...
# --------------------------------------------------- Figura B2 (eje dual)
def plot_steps_and_accuracy_by_difficulty(adapt, out_path, FS):
    ...
    l1, = ax1.plot(idx, acc, color=BLUE, marker="o", markersize=7.5, linewidth=2.4,
                   markeredgecolor="white", markeredgewidth=1.1,
                   zorder=3, label="Accuracy (%)")
    l2, = ax2.plot(idx, stp, color=ORANGE, marker="s", markersize=7.5, linewidth=2.4,
                   markeredgecolor="white", markeredgewidth=1.1,
                   zorder=3, label="Latent tokens used")

    ax1.set_xlabel(r"Dataset reasoning steps (difficulty)", fontsize=FS["label"], color=TEXT)
    ax1.set_ylabel("Accuracy (%)", fontsize=FS["label"], color=BLUE)
    ax2.set_ylabel("Latent tokens used (avg.)", fontsize=FS["label"], color=ORANGE)

    ax1.set_xticks(idx)
    ax1.set_xticklabels(labels)
    ax1.set_ylim(0, 70)
    ax2.set_ylim(0, 6)
    ax1.tick_params(axis="x", colors=TEXT, labelsize=FS["tick"])
    ax1.tick_params(axis="y", colors=BLUE, labelsize=FS["tick"])
    ax2.tick_params(axis="y", colors=ORANGE, labelsize=FS["tick"])
    ...
    ax1.legend(handles=[l1, l2], loc="upper center", frameon=False, fontsize=FS["legend"],
...
# ---------------------------------------------------------------- Figura C
def plot_joint_matrix(adapt, out_path, FS, max_cot=7, max_steps=8):
    ...
    im = ax.imshow(Mn, aspect="auto", cmap=figstyle.BLUES, origin="lower",
                   vmin=0, vmax=Mn.max())
    ...
    ax.set_xlabel("Latent tokens used by the model", fontsize=FS["label"], color=TEXT)
    ax.set_ylabel(r"Dataset steps (difficulty)", fontsize=FS["label"], color=TEXT)
    ax.tick_params(colors=TEXT, labelsize=FS["tick"])
    for spine in ax.spines.values():
        spine.set_color(GRID)

    ax.text(0.97, 0.05, rf"Spearman $= {rho:+.3f}$", transform=ax.transAxes,
            ha="right", va="bottom", fontsize=FS["tick"], color=TEXT,
            bbox=dict(boxstyle="round,pad=0.3", fc="white", ec=GRID, lw=0.8))

    cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.03)
    cbar.set_label("Per-row fraction  $P(\\mathrm{steps}\\mid\\mathrm{difficulty})$",
                   fontsize=FS["tick"] * 0.94, color=TEXT)
    cbar.ax.tick_params(colors=TEXT, labelsize=FS["tick"] * 0.9)
...
# ---------------------------------------------------------------- Figura D
def plot_steps_used_distribution(adapt, out_path, FS, k_fixed=6):
    ...
    ax.set_xlabel("Latent tokens used", fontsize=FS["label"], color=TEXT)
    ax.set_ylabel("Fraction of the test set", fontsize=FS["label"], color=TEXT)
    ax.set_xticks(xs)
    style_axes(ax, FS["tick"])
    fig.tight_layout()
    fig.savefig(out_path, dpi=200)
...
if __name__ == "__main__":
    ...
    p.add_argument("--profile", choices=["paper", "poster"], default="paper")
    args = p.parse_args()
    ...
    plot_accuracy_by_difficulty(adapt, base, args.out_dir / "accuracy_by_difficulty.png", FS)
    plot_steps_by_difficulty(adapt, base, args.out_dir / "steps_by_difficulty.png", FS)
    plot_steps_and_accuracy_by_difficulty(adapt, args.out_dir / "steps_accuracy_by_difficulty.png", FS)
    plot_joint_matrix(adapt, args.out_dir / "joint_matrix.png", FS)
    plot_steps_used_distribution(adapt, args.out_dir / "steps_used_distribution.png", FS)
