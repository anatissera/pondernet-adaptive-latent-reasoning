...
def plot(d, out_path, profile="poster"):
    ...
    knee_s, knee_a = 3.011, 39.88  # M2 @ threshold 0.5
    ax.annotate("Adaptive model\n3 steps", xy=(knee_s, knee_a),
                xytext=(2.02, 40.55), ha="left", va="center",
                fontsize=23, color=BLUE,
                arrowprops=dict(arrowstyle="-", color=BLUE, lw=1.3))
    ax.annotate("Fixed-$K$ baseline\n6 steps", xy=(base_s, base_a),
                xytext=(5.55, 38.55), ha="center", va="center",
                fontsize=23, color=RED,
                arrowprops=dict(arrowstyle="-", color=RED, lw=1.3))

    # savings arrow between the two comparable points
    ax.annotate("", xy=(knee_s + 0.15, base_a), xytext=(base_s - 0.28, base_a),
                arrowprops=dict(arrowstyle="-|>", color=TEXT, lw=1.9,
                                shrinkA=0, shrinkB=0), zorder=7)
    ax.text((knee_s + base_s) / 2, base_a + 0.13, "$-$50% compute",
            ha="center", va="bottom", fontsize=23, color=TEXT)

    ax.set_xlabel("Latent steps per problem (compute)", fontsize=21, color=TEXT)
