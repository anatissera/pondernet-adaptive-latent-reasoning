...
def plot_accuracy_by_difficulty(adapt, base, out_path):
    ...
    ax.set_xlabel(r"Dataset reasoning steps (difficulty)", fontsize=10, color=TEXT)
...
def plot_steps_by_difficulty(adapt, base, out_path, k_fixed=6):
    ...
    ax.set_xticklabels(labels)
    ax.set_xlabel(r"Dataset reasoning steps (difficulty)", fontsize=10, color=TEXT)
    ax.set_ylabel("Latent tokens used (avg.)", fontsize=10, color=TEXT)
...
def plot_steps_and_accuracy_by_difficulty(adapt, out_path):
    ...
    ax1.set_xlabel(r"Dataset reasoning steps (difficulty)", fontsize=10, color=TEXT)
    ax1.set_ylabel("Accuracy (%)", fontsize=10, color=PURPLE)
    ax2.set_ylabel("Latent tokens used (avg.)", fontsize=10, color=BLUE)
...
def plot_joint_matrix(adapt, out_path, max_cot=7, max_steps=8):
    ...
    ax.set_xlabel("Latent tokens used by the model", fontsize=10, color=TEXT)
    ax.set_ylabel(r"Dataset steps (difficulty)", fontsize=10, color=TEXT)
    ax.tick_params(colors=TEXT, labelsize=9)
...
def plot_steps_used_distribution(adapt, out_path, k_fixed=6):
    ...
    ax.set_xlabel("Latent tokens used", fontsize=10, color=TEXT)
    ax.set_ylabel("Fraction of the test set", fontsize=10, color=TEXT)
    ax.set_xticks(xs)
    style_axes(ax)
    fig.tight_layout()
    fig.savefig(out_path, dpi=200)
