...
def plot_kstar(out_path: Path) -> None:
    ...
    ax.set_xlabel(r"$k^{*}$ optimal per instance", fontsize=FS_LABEL,
                  color=figstyle.TEXT)
    ax.set_ylabel("Examples (%)", fontsize=FS_LABEL, color=figstyle.TEXT)
    ax.set_xticks(KS)
    ax.set_ylim(0, 80)
...
def plot_accuracy(out_path: Path) -> None:
    ...
    ax.plot(KS, [a * 100 for a in ACC_200], color=figstyle.ACCENT, marker="D",
            ...
            label=r"Pilot ($n{=}200$)")
    ax.set_xlabel(r"$k$ (forced steps)", fontsize=FS_LABEL, color=figstyle.TEXT)
